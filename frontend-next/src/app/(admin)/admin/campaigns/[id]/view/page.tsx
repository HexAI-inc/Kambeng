      ...
      <div style={{ maxWidth: 860, margin: "0 auto", display: "flex", flexDirection: "column", gap: 20 }}>
        ...
        <motion.div initial={{ opacity: 0, y: 14 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.35 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 4 }}>
            ...
            <div style={{ fontSize: 22, fontWeight: 900, color: "#15201a", letterSpacing: "-0.03em" }}>{campaign.title}</div>
            <StatusChip status={String(campaign.status)} />
          </div>
          <div style={{ fontSize: 13, color: "#626d66", marginLeft: 46 }}>{campaign.slug}</div>
        </motion.div>
        ...
        <div style={{ background: "#ffffff", border: "1px solid rgba(21,32,26,0.07)", borderRadius: 16, padding: "24px" }}>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 20 }}>
            ...
            {campaign.target_amount && (
              <div style={{ gridColumn: "1 / -1" }}>
                <div style={{ fontSize: 11, fontWeight: 700, color: "#6e7872", textTransform: "uppercase", letterSpacing: "0.07em", marginBottom: 8 }}>Progress</div>
                <div style={{ height: 8, background: "rgba(21,32,26,0.08)", borderRadius: 4, overflow: "hidden" }}>
                  <div style={{ height: "100%", width: `${progress}%`, background: `linear-gradient(90deg, ${BLUE}, ${GREEN})`, borderRadius: 4, transition: "width 0.5s" }} />
                </div>
                <div style={{ fontSize: 12, color: "#6e7872", marginTop: 4 }}>{Math.round(progress)}% of goal</div>
              </div>
            )}
            {campaign.description && (
              <div style={{ gridColumn: "1 / -1" }}>
                <Field label="Description" value={<p style={{ color: "#56625b", lineHeight: 1.7, margin: 0 }}>{campaign.description}</p>} />
              </div>
            )}
          </div>
        </div>
        ...
        <div style={{ display: "flex", gap: 10, flexWrap: "wrap" }}>
          <button onClick={() => router.push(`/admin/campaigns/${campaign.id}/edit`)} style={{ padding: "10px 20px", borderRadius: 9, border: "none", background: `${BLUE}`, color: "#fff", fontSize: 13, fontWeight: 700, cursor: "pointer" }}>Edit Campaign</button>
