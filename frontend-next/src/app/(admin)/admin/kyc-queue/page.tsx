  ...
  return (
    <div style={{ minHeight: "100vh", padding: "28px clamp(16px,4vw,48px)" }}>
      <div style={{ maxWidth: 1000, margin: "0 auto", display: "flex", flexDirection: "column", gap: 20 }}>
        ...
        {/* Header */}
        <motion.div {...fadeUp(0)}>
          <div style={{ fontSize: 22, fontWeight: 900, color: "#15201a", letterSpacing: "-0.03em" }}>KYC Queue</div>
          <div style={{ fontSize: 13, color: "#626d66", marginTop: 4 }}>
            {rows.length} total · <span style={{ color: pending > 0 ? "#e8650f" : "#6e7872" }}>{pending} pending review</span>
          </div>
        </motion.div>
        ...
        {rejectingId && (
          <motion.div initial={{ opacity: 0, scale: 0.97 }} animate={{ opacity: 1, scale: 1 }} style={{
            ...
            <div style={{ fontSize: 15, fontWeight: 700, color: "#15201a", marginBottom: 4 }}>
              Reject KYC — {rows.find((r) => r.id === rejectingId)?.user_name ?? `Submission ${rejectingId}`}
            </div>
            <div style={{ fontSize: 13, color: "#626d66", marginBottom: 16 }}>Provide a reason — this will be sent to the user.</div>
            <textarea
          ...
          <div style={{ background: "#ffffff", border: "1px solid rgba(21,32,26,0.07)", borderRadius: 16, overflowX: "auto" }}>
            ...
            {isLoading ? (
              <div style={{ padding: 40, textAlign: "center" }}>
                ...
                <div style={{ color: "#6e7872", fontSize: 13 }}>Loading KYC queue…</div>
              </div>
            ) : rows.length === 0 ? (
              <div style={{ padding: "48px 24px", textAlign: "center", color: "#6e7872", fontSize: 14 }}>No KYC submissions</div>
              ...
              return (
                ...
                >
                  <div data-label="User">
                    <div style={{ fontSize: 13, fontWeight: 700, color: "#15201a" }}>{sub.user_name}</div>
                    <div style={{ fontSize: 11, color: "#6e7872" }}>{sub.user_email}</div>
                  </div>
                  <div data-label="Status"><StatusChip status={sub.status} /></div>
                  <div data-label="Submitted" style={{ fontSize: 12, color: "#56625b" }}>{new Date(sub.created_at).toLocaleDateString("en-GB", { day: "numeric", month: "short", year: "numeric" })}</div>
                  <div data-label="Docs" style={{ fontSize: 13, color: "#56625b", fontWeight: 600 }}>{sub.document_file_url ? "1 doc" : "0 docs"}</div>
