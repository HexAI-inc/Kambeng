...
const STATUS_TABS = [
  ...
  { key: "DISMISSED", label: "Dismissed", color: "#56625b" },
    ...
    <div style={{ background: "#f6f4ef", minHeight: "100vh", padding: "28px clamp(16px,4vw,48px)" }}>
      <div style={{ maxWidth: 1100, margin: "0 auto", display: "flex", flexDirection: "column", gap: 20 }}>
        ...
        <motion.div {...fadeUp(0.06)}>
          <div style={{ background: "#ffffff", border: "1px solid rgba(21,32,26,0.07)", borderRadius: 14, padding: "18px 20px" }}>
            ...
            <div style={{ display: "flex", flexWrap: "wrap", gap: 6 }}>
              <span style={{ fontSize: 11, color: "#6e7872", fontWeight: 700, alignSelf: "center", marginRight: 4 }}>FILTER:</span>
              {MODELS.map((m) => {
                const active = selectedModels.includes(m);
                return (
                  <button key={m} onClick={() => setSelectedModels((prev) => active ? prev.filter((x) => x !== m) : [...prev, m])}
                    style={{ padding: "4px 12px", borderRadius: 20, fontSize: 11, fontWeight: 700, cursor: "pointer", border: `1px solid ${active ? "rgba(20,120,74,0.3)" : "rgba(21,32,26,0.08)"}`, background: active ? "rgba(20,120,74,0.1)" : "rgba(21,32,26,0.03)", color: active ? BLUE : "#6e7872", textTransform: "capitalize" }}>
                    {m}
                  </button>
                );
              })}
          ...
          <motion.div {...fadeUp(0.1)}>
            <div style={{ background: "#ffffff", border: "1px solid rgba(21,32,26,0.07)", borderRadius: 16, overflowX: "auto" }}>
              ...
              <div className="admin-table-wrap" style={{ minWidth: 660 }}>
                <div className="admin-table-header" style={{ display: "grid", gridTemplateColumns: "100px 1fr 1fr 100px 80px 80px", padding: "10px 18px", borderBottom: "1px solid rgba(21,32,26,0.06)", fontSize: 10, fontWeight: 700, color: "#6e7872", textTransform: "uppercase", letterSpacing: "0.07em" }}>
                ...
                {search.isLoading ? (
                  <div style={{ padding: 32, textAlign: "center" }}>
                    ...
                    <div style={{ color: "#6e7872", fontSize: 13 }}>Searching…</div>
                  </div>
                ) : (search.data?.items ?? []).length === 0 ? (
                  <div style={{ padding: "40px 24px", textAlign: "center", color: "#6e7872", fontSize: 14 }}>No results found</div>
                ) : (search.data?.items ?? []).map((item: AdminSearchResultItem, i: number) => (
                  <div key={i} className="admin-table-row" style={{ display: "grid", gridTemplateColumns: "100px 1fr 1fr 100px 80px 80px", padding: "12px 18px", alignItems: "center", borderBottom: "1px solid rgba(21,32,26,0.04)", transition: "background 0.15s" }}
        ...
        <motion.div id="moderation-queue" {...fadeUp(0.12)}>
          <div style={{ background: "#ffffff", border: "1px solid rgba(21,32,26,0.08)", borderRadius: 16, padding: "22px 22px 26px" }}>
            <div style={{ marginBottom: 16 }}>
              <div style={{ fontSize: 16, fontWeight: 800, color: "#15201a" }}>Moderation Queue</div>
              <div style={{ fontSize: 12, color: "#626d66", marginTop: 4 }}>{reportRows.length} report{reportRows.length !== 1 ? "s" : ""} in view</div>
            </div>
              ...
              {STATUS_TABS.map(({ key, label, color }) => {
                ...
                return (
                  <button key={key} onClick={() => setStatusTab(key)}
                    style={{ padding: "6px 16px", borderRadius: 20, fontSize: 12, fontWeight: 700, cursor: "pointer", border: `1px solid ${active ? `${color}50` : "rgba(21,32,26,0.08)"}`, background: active ? `${color}15` : "rgba(21,32,26,0.03)", color: active ? color : "#6e7872", transition: "all 0.15s" }}>
                    {label}
                  </button>
                );
              })}
            ...
            {resolvingId && resolvingReport && (
              <div style={{ background: "rgba(21,32,26,0.02)", border: "1px solid rgba(21,32,26,0.1)", borderRadius: 14, padding: 20, marginBottom: 18 }}>
                ...
                <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 12 }}>
                  ...
                  <span style={{ fontSize: 11, color: "#6e7872", marginLeft: "auto" }}>Report #{resolvingReport.id}</span>
                </div>

                {resolvingReport.description && (
                  <div style={{ background: "#f6f4ef", border: "1px solid rgba(21,32,26,0.06)", borderRadius: 9, padding: "10px 14px", marginBottom: 14, fontSize: 13, color: "#4d5a52", lineHeight: 1.6, fontStyle: "italic" }}>
                ...
                <div style={{ display: "flex", gap: 10, marginBottom: 16, flexWrap: "wrap", alignItems: "center" }}>
                  ...
                  {resolvingReport.reported_by_user_id && (
                    <span style={{ fontSize: 12, color: "#6e7872" }}>Reporter UID: {resolvingReport.reported_by_user_id}</span>
                  )}
                  <span style={{ fontSize: 12, color: "#6e7872" }}>Entity ID: {resolvingReport.reported_entity_id}</span>
                </div>

                <div style={{ fontSize: 12, color: "#626d66", fontWeight: 600, marginBottom: 10 }}>Select an action:</div>
                {(() => {
                    ...
                    <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(190px, 1fr))", gap: 8, marginBottom: 16 }}>
                      {availableActions.map(([key, { label, description, color, bg, border }]) => (
                        <button key={key} onClick={() => setSelectedAction(key)}
                          style={{ padding: "11px 14px", borderRadius: 10, fontSize: 13, fontWeight: 700, cursor: "pointer", border: `1px solid ${selectedAction === key ? border : "rgba(21,32,26,0.08)"}`, background: selectedAction === key ? bg : "rgba(21,32,26,0.03)", color: selectedAction === key ? color : "#56625b", textAlign: "left" }}>
                          <div>{label}</div>
                ...
                <div style={{ display: "flex", gap: 10 }}>
                  <button onClick={() => void handleResolve()} disabled={!selectedAction || resolveReport.isPending}
                    style={{ padding: "10px 22px", borderRadius: 9, border: "none", background: selectedAction ? `${BLUE}` : "rgba(21,32,26,0.06)", color: selectedAction ? "#fff" : "#6e7872", fontSize: 13, fontWeight: 700, cursor: selectedAction ? "pointer" : "not-allowed" }}>
            ...
            <div style={{ background: "#ffffff", border: "1px solid rgba(21,32,26,0.07)", borderRadius: 16, overflowX: "auto" }}>
              <div className="admin-table-wrap" style={{ minWidth: 700 }}>
                <div className="admin-table-header" style={{ display: "grid", gridTemplateColumns: "140px 100px 90px 1fr 180px 80px", padding: "10px 18px", borderBottom: "1px solid rgba(21,32,26,0.06)", fontSize: 10, fontWeight: 700, color: "#6e7872", textTransform: "uppercase", letterSpacing: "0.07em" }}>
                ...
                {reportsLoading ? (
                  <div style={{ padding: 40, textAlign: "center" }}>
                    ...
                    <div style={{ color: "#6e7872", fontSize: 13 }}>Loading reports…</div>
                  </div>
                ) : reportRows.length === 0 ? (
                  ...
                  const statusColor = isOpen ? RED : isResolved ? GREEN : "#626d66";
                  const isDismissConfirm = dismissConfirmId === r.id;
                  return (
                    <div key={r.id} className="admin-table-row"
                      style={{ display: "grid", gridTemplateColumns: "140px 100px 90px 1fr 180px 80px", padding: "13px 18px", alignItems: "center", borderBottom: "1px solid rgba(21,32,26,0.04)", borderLeft: isOpen ? `3px solid ${RED}` : "3px solid transparent", transition: "background 0.15s" }}
                      onMouseEnter={(e) => { (e.currentTarget as HTMLDivElement).style.background = "rgba(21,32,26,0.02)"; }}
                      onMouseLeave={(e) => { (e.currentTarget as HTMLDivElement).style.background = ""; }}
                    >
                      <div data-label="Reason" style={{ fontSize: 12, color: "#56625b", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{r.reason?.replace(/_/g, " ")}</div>
                      ...
                      <div data-label="Description" style={{ overflow: "hidden" }}>
                        <div style={{ fontSize: 12, color: "#626d66", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{r.description || "—"}</div>
                        {!isOpen && r.action_taken && (
                          <div style={{ fontSize: 11, color: "#6e7872", marginTop: 2 }}>Action: {r.action_taken.replace(/_/g, " ")}</div>
                        )}
                      </div>
                      ...
                      <div data-label="Date" style={{ fontSize: 11, color: "#6e7872" }}>{new Date(r.created_at).toLocaleDateString("en-GB", { day: "numeric", month: "short" })}</div>
                    </div>
                  );
                })}
              </div>
