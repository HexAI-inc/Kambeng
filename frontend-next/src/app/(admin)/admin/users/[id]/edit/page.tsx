    ...
    <div style={{ background: "#f6f4ef", minHeight: "100vh", padding: "28px clamp(16px,4vw,48px)" }}>
      <div style={{ maxWidth: 720, margin: "0 auto", display: "flex", flexDirection: "column", gap: 20 }}>
        ...
        <motion.div {...fade(0)} style={{ display: "flex", alignItems: "center", gap: 12 }}>
          ...
          <div>
            <div style={{ fontSize: 22, fontWeight: 900, color: "#15201a", letterSpacing: "-0.03em" }}>Edit User #{userId}</div>
            <div style={{ fontSize: 13, color: "#626d66", marginTop: 2 }}>{user.email}</div>
          </div>
        </motion.div>
        ...
        <motion.div {...fade(0.05)}>
          <div style={{ background: "#ffffff", border: "1px solid rgba(21,32,26,0.07)", borderRadius: 16, padding: "24px" }}>
            <div style={{ fontSize: 14, fontWeight: 700, color: "#15201a", marginBottom: 18 }}>Identity</div>
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14 }}>
              <div style={{ gridColumn: "1 / -1" }}>
                <label style={{ fontSize: 10, fontWeight: 700, color: "#6e7872", textTransform: "uppercase", letterSpacing: "0.07em", display: "block", marginBottom: 6 }}>Full name</label>
              ...
              <div>
                <label style={{ fontSize: 10, fontWeight: 700, color: "#6e7872", textTransform: "uppercase", letterSpacing: "0.07em", display: "block", marginBottom: 6 }}>Email</label>
                <input value={form.email} onChange={(e) => set({ email: e.target.value })} type="email" style={inputStyle}
                  onFocus={(e) => { e.currentTarget.style.borderColor = "rgba(20,120,74,0.4)"; }}
                  onBlur={(e) => { e.currentTarget.style.borderColor = "rgba(21,32,26,0.1)"; }} />
              </div>
              <div>
                <label style={{ fontSize: 10, fontWeight: 700, color: "#6e7872", textTransform: "uppercase", letterSpacing: "0.07em", display: "block", marginBottom: 6 }}>Wave number</label>
        ...
        <motion.div {...fade(0.08)}>
          <div style={{ background: "#ffffff", border: "1px solid rgba(21,32,26,0.07)", borderRadius: 16, padding: "24px" }}>
            <div style={{ fontSize: 14, fontWeight: 700, color: "#15201a", marginBottom: 6 }}>Role</div>
            <div style={{ fontSize: 12, color: "#626d66", marginBottom: 14 }}>ADMIN can access the admin panel and manage all users and campaigns.</div>
            <div style={{ display: "flex", gap: 8 }}>
              {(["USER", "ADMIN"] as const).map((r) => (
                <button key={r} onClick={() => set({ role: r })} style={{
                  padding: "9px 22px", borderRadius: 9, fontSize: 13, fontWeight: 700, cursor: "pointer",
                  border: `1px solid ${form.role === r ? (r === "ADMIN" ? "rgba(232,101,15,0.4)" : "rgba(20,120,74,0.4)") : "rgba(21,32,26,0.1)"}`,
                  background: form.role === r ? (r === "ADMIN" ? "rgba(232,101,15,0.12)" : "rgba(20,120,74,0.1)") : "rgba(21,32,26,0.04)",
                  color: form.role === r ? (r === "ADMIN" ? "#e8650f" : BLUE) : "#56625b",
                }}>{r}</button>
              ))}
            </div>
          </div>
        ...
        <motion.div {...fade(0.1)}>
          <div style={{ background: "#ffffff", border: "1px solid rgba(21,32,26,0.07)", borderRadius: 16, padding: "24px" }}>
            <div style={{ fontSize: 14, fontWeight: 700, color: "#15201a", marginBottom: 14 }}>Account status</div>
            <div style={{ display: "flex", gap: 8 }}>
              {([true, false] as const).map((active) => (
                <button key={String(active)} onClick={() => set({ is_active: active })} style={{
                  padding: "9px 22px", borderRadius: 9, fontSize: 13, fontWeight: 700, cursor: "pointer",
                  border: `1px solid ${form.is_active === active ? (active ? "rgba(31,153,96,0.4)" : "rgba(239,68,68,0.4)") : "rgba(21,32,26,0.1)"}`,
                  background: form.is_active === active ? (active ? "rgba(31,153,96,0.12)" : "rgba(239,68,68,0.12)") : "rgba(21,32,26,0.04)",
                  color: form.is_active === active ? (active ? GREEN : RED) : "#56625b",
        ...
        <motion.div {...fade(0.12)}>
          <div style={{ background: "#ffffff", border: "1px solid rgba(21,32,26,0.07)", borderRadius: 16, padding: "24px" }}>
            <div style={{ fontSize: 14, fontWeight: 700, color: "#15201a", marginBottom: 6 }}>KYC status override</div>
            <div style={{ fontSize: 12, color: "#626d66", marginBottom: 14 }}>Use with care — setting APPROVED grants withdrawal access without a document review.</div>
              ...
              {KYC_STATUSES.map((s) => {
                ...
                return (
                  <button key={s} onClick={() => set({ kyc_status: s })} style={{
                    ...
                    border: `1px solid ${active ? `${c}66` : "rgba(21,32,26,0.1)"}`,
                    background: active ? `${c}18` : "rgba(21,32,26,0.04)",
                    color: active ? c : "#56625b",
                  }}>{s.replace("_", " ")}</button>
                );
              })}
            </div>
