...
function QRCard({ title, description, tag, tagColor, qrData, isLoading, onDownload, onCopy }: {
  ...
  return (
    <div style={{
      ...
      onMouseEnter={(e) => { (e.currentTarget as HTMLDivElement).style.borderColor = "rgba(20,120,74,0.2)"; }}
    ...
    >
      ...
      <div style={{ padding: "20px" }}>
        ...
        <div style={{ fontSize: 14, fontWeight: 700, color: "#15201a", marginBottom: 4 }}>{title}</div>
        <div style={{ fontSize: 12, color: "#626d66", marginBottom: 16, lineHeight: 1.6 }}>{description}</div>

        {/* QR image */}
        <div style={{
          width: "100%", aspectRatio: "1", maxWidth: 200, margin: "0 auto 16px",
          background: isLoading ? "rgba(21,32,26,0.04)" : "#fff",
          ...
          {isLoading ? (
            ...
            <div style={{ fontSize: 12, color: "#6e7872", textAlign: "center", padding: 16 }}>Not available</div>
          )}
        </div>
        ...
        {qrData && (
          <div style={{ display: "flex", gap: 8 }}>
            <button onClick={onDownload} style={{
              ...
              boxShadow: "0 2px 10px rgba(20,120,74,0.25)",
            }}>Download</button>
    ...
    <div style={{ minHeight: "100vh", padding: "28px clamp(16px, 4vw, 48px)" }}>
      <div style={{ maxWidth: 1100, margin: "0 auto", display: "flex", flexDirection: "column", gap: 24 }}>
        ...
        <motion.div {...fadeUp(0)}>
          ...
          <div style={{ fontSize: 22, fontWeight: 900, color: "#15201a", letterSpacing: "-0.03em", marginBottom: 4 }}>QR Codes</div>
          <div style={{ fontSize: 13, color: "#626d66" }}>Share these QR codes to help people discover and donate to your campaign.</div>
        </motion.div>
        ...
        <motion.div {...fadeUp(0.12)}>
          <div style={{ fontSize: 11, color: "#6e7872", fontWeight: 700, letterSpacing: "0.08em", textTransform: "uppercase" as const, marginBottom: 12 }}>Campaign Links</div>
        ...
        <motion.div {...fadeUp(0.16)}>
          <div style={{ fontSize: 11, color: "#6e7872", fontWeight: 700, letterSpacing: "0.08em", textTransform: "uppercase" as const, marginBottom: 12 }}>Tips</div>
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(200px, 1fr))", gap: 10 }}>
            {tips.map(({ icon, title, desc }) => (
              <div key={title} style={{
                ...
                <div style={{ fontSize: 12, color: "#626d66", lineHeight: 1.65 }}>{desc}</div>
              </div>
            ))}
          </div>
        </motion.div>
