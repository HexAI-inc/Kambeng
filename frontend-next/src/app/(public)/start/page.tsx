    ...
    <div style={{ padding: "48px clamp(16px, 5vw, 72px) 96px" }}>
      <div className="start-grid" style={{ maxWidth: 1100, margin: "0 auto", display: "grid", gridTemplateColumns: "1fr 1fr", gap: 48, alignItems: "start" }}>
        ...
        <div>
          ...
          <h1 style={{ fontSize: "clamp(28px, 4vw, 42px)", fontWeight: 900, color: "#15201a", letterSpacing: "-0.04em", lineHeight: 1.1, margin: "0 0 16px" }}>
            Raise money for your community — with proof donors can trust
          </h1>
          <p style={{ color: "#56625b", fontSize: 15, lineHeight: 1.8, marginBottom: 36 }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
            {STEPS.map((step, i) => (
              <div key={step.title} style={{ display: "flex", gap: 14, padding: "16px 18px", borderRadius: 14, background: "#ffffff", border: "1px solid rgba(21,32,26,0.07)" }}>
                ...
                <div>
                  <div style={{ fontSize: 14, fontWeight: 700, color: "#15201a", marginBottom: 4 }}>{step.title}</div>
                  <div style={{ fontSize: 13, color: "#56625b", lineHeight: 1.65 }}>{step.detail}</div>
                </div>
              </div>
            ))}
          </div>

          <div style={{ marginTop: 24, fontSize: 13, color: "#626d66", lineHeight: 1.7 }}>
        ...
        {/* Right: waitlist form */}
        <div style={{ position: "sticky", top: 88, background: "#ffffff", border: "1px solid rgba(20,120,74,0.18)", borderRadius: 20, padding: "28px clamp(20px, 3vw, 32px) 32px" }}>
          {status === "done" ? (
            <div style={{ textAlign: "center", padding: "24px 0" }}>
              ...
              <div style={{ fontSize: 18, fontWeight: 800, color: "#15201a", marginBottom: 8 }}>You&apos;re on the list!</div>
              <p style={{ fontSize: 13, color: "#56625b", lineHeight: 1.7, marginBottom: 20 }}>
                Check your inbox to confirm your email — then we&apos;ll walk you through everything, step by step. Want to skip ahead?
              </p>
              <Link href="/auth/signup" style={{ display: "inline-block", padding: "12px 24px", borderRadius: 10, background: `${BLUE}`, color: "#fff", fontSize: 14, fontWeight: 700, textDecoration: "none" }}>
                Create your account now
              </Link>
            </div>
          ) : (
            <form onSubmit={submit}>
              <div style={{ fontSize: 18, fontWeight: 800, color: "#15201a", marginBottom: 6 }}>Join the organiser waitlist</div>
              <p style={{ fontSize: 13, color: "#56625b", lineHeight: 1.65, marginBottom: 22 }}>
                Tell us a little about your cause and we&apos;ll help you get set up — personally.
              </p>
              <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
                <div>
                  <label style={{ display: "block", fontSize: 12, fontWeight: 600, color: "#56625b", marginBottom: 6 }}>Your name</label>
                  <input required value={name} onChange={(e) => setName(e.target.value)} placeholder="e.g. Fatou Ceesay" style={inputStyle} />
                </div>
                <div>
                  <label style={{ display: "block", fontSize: 12, fontWeight: 600, color: "#56625b", marginBottom: 6 }}>Email</label>
                  <input required type="email" value={email} onChange={(e) => setEmail(e.target.value)} placeholder="you@example.com" style={inputStyle} />
                </div>
                <div>
                  <label style={{ display: "block", fontSize: 12, fontWeight: 600, color: "#56625b", marginBottom: 6 }}>Phone <span style={{ fontWeight: 400, color: "#6e7872" }}>(optional)</span></label>
                ...
                <div>
                  <label style={{ display: "block", fontSize: 12, fontWeight: 600, color: "#56625b", marginBottom: 6 }}>What do you want to raise funds for?</label>
                ...
                <button
                  ...
                  style={{ marginTop: 4, padding: "14px 20px", borderRadius: 12, border: "none", background: `${BLUE}`, color: "#fff", fontSize: 15, fontWeight: 700, cursor: status === "sending" ? "wait" : "pointer", opacity: status === "sending" ? 0.7 : 1, boxShadow: "0 6px 24px rgba(20,120,74,0.3)" }}
                >
                  {status === "sending" ? "Joining…" : "Join the waitlist"}
                </button>
                {error && <div style={{ fontSize: 13, color: "#d42f2f" }}>{error}</div>}
                <div style={{ fontSize: 11, color: "#6e7872", lineHeight: 1.6 }}>
