...
function InfoRow({ label, value }: { label: string; value: string }) {
  return (
    <div style={{ display: "flex", justifyContent: "space-between", gap: 16, padding: "12px 0", borderBottom: "1px solid rgba(21,32,26,0.06)" }}>
      <div style={{ fontSize: 12, color: "#56625b", fontWeight: 600 }}>{label}</div>
      <div style={{ fontSize: 13, color: "#15201a", fontWeight: 600, textAlign: "right", wordBreak: "break-word" }}>{value}</div>
    </div>
  );
}
...
export function OnboardingCard({ fullName, email, waveNumber }: OnboardingCardProps) {
  ...
  return (
    <div style={{
      ...
      <div style={{ position: "absolute", width: 640, height: 640, borderRadius: "50%", background: "radial-gradient(circle, rgba(20,120,74,0.10) 0%, transparent 70%)", right: "-18%", top: "-18%", pointerEvents: "none" }} />
      <div style={{ position: "absolute", width: 420, height: 420, borderRadius: "50%", background: "radial-gradient(circle, rgba(31,153,96,0.07) 0%, transparent 70%)", left: "-12%", bottom: "-12%", pointerEvents: "none" }} />
      ...
      <div style={{ width: "100%", maxWidth: 560, position: "relative", zIndex: 1 }}>
        ...
        <div style={{ background: "#ffffff", border: "1px solid rgba(21,32,26,0.08)", borderRadius: 24, padding: 32, boxShadow: "0 24px 80px rgba(21,32,26,0.12)" }}>
          ...
          <div style={{ fontSize: 28, fontWeight: 900, color: "#15201a", letterSpacing: "-0.04em", lineHeight: 1.1, marginBottom: 10 }}>
            Welcome{fullName ? `, ${fullName}` : ""}.
          </div>
          <div style={{ fontSize: 14, color: "#626d66", lineHeight: 1.7, marginBottom: 24 }}>
